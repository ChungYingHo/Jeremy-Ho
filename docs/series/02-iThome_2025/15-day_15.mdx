...
sidebar_label: "Day15 - Waiter! 然後 API 端上了一盤 response"
tags: [JavaScript, api]
unlisted: true
...
在 AJAX 技術出現之前，前端主要還是使用同步的方式來獲取資料，所以當時的網站 UX 相當不佳。  
...
SOAP 的傳輸格式也是較早期流行的 XML (不知道 XML 的可以看[我的 python 筆記](https://jeremyho.tw/docs/notes/Python/access_web_data#xml))，這也讓它在當時的網路環境下有著較好的跨平台能力。  
