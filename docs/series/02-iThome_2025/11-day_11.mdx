...
sidebar_label: "Day11 - needTypeCheck ? TypeScript : JavaScript"
unlisted: true
...
眾所周知 Javascript 其實是個有別於其他程式語言的動態型別語言，所謂動態就是寫 code 時不用特別塞給變數一個 type，JS 會自己去推斷，這樣的好處是相當自由，一個變數現在想是 string 就是 string，想是 object 就是 object。  
...
我更喜歡說 TypeScript 是 JavaScript 的強型別寫法。  
這篇也不是在教如何使用 TypeScript，想從 0 開始學的建議翻一下 TS 的官網，或是可以稍微瞄一下我以前寫的[筆記](https://jeremyho.tw/docs/category/typescript)。  
...
先不論它是否真的有這樣說過，但 web 圈確實 JS 跟 TS 各自都有擁護者。  
