...
2. 立即函式會印出 1
3. `addK('A')` 會印出 `Await A: 2`
4. `addK('B')` 會印出 `Await B: 2`
5. 最後才是 `setTimeout` 會印出 3

這題的癥結點有兩個。  
一個是 async/await 跟 `setTimeout` 的優先權，但透過前面有說過的， async/await 是 microtask，其優先權會高於 macrotask 的 `setTimeout`，所以還好。  
最有問題的是 `addK('A')` 跟 `addK('B')` 這兩個像的印出結果為何都是 2。難道 `addK('A')` 不會把 `k` 加 1，然後影響 `addK('B')` 嗎？  
這就是 await 妙的地方。  

對 await 來說，`await k` 等同 `await Promise.resolve(k)`，它會抓當下的 `k` 值，然後把 `+ 1` 丟到 microtask queue，等到下一輪事件循環再執行。  
所以 `addK('A')` 跟 `addK('B')` 的執行邏輯其實是：
1. 同步部分跑完 (把兩個 `addK` 都呼叫一遍，兩個人都已經存了「快照 0」，續行排進 microtask queue)。
2. event loop 開始跑 microtask queue：
    - 先跑 A 的續行 → 用快照 0 → 算出 1 → 寫回 k=1。
    - 再跑 B 的續行 → 它也用自己存的快照 0 → 算出 1 → 再寫回 k=1。
...
sequenceDiagram
    ...
    Main->>A: 呼叫 addK('A')
    A->>A: snapshotA = k (=1)
    A->>MQ: 推入續行 (使用 snapshotA)

    Main->>B: 呼叫 addK('B')
    B->>B: snapshotB = k (=1)
    B->>MQ: 推入續行 (使用 snapshotB)

    Note over Main: 同步程式碼結束

    MQ->>A: 執行續行 (k = snapshotA + 1 = 2)
    A->>Main: 更新 k = 2

    MQ->>B: 執行續行 (k = snapshotB + 1 = 2)
    B->>Main: 再次更新 k = 2
```