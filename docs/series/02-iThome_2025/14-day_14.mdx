...
或是我以前也寫過一篇更詳細在講非同步的[文章](https://jeremyho.tw/docs/languages/JavaScript/asynchronous)，那篇有更多關於非同步的詳細解釋。
...
sequenceDiagram
    ...
    participant A as addK('A')
    participant IIFE as (IIFE)
    participant B as addK('B')
    participant MQ as Microtask Queue
    participant TQ as Macrotask Queue (setTimeout)

    Note over Main: 初始 k = 0

    Main->>TQ: 排入 setTimeout(…, 0)
    Main->>Main: console.log(k) → 0

    Main->>A: 呼叫 addK('A')
    A->>A: snapshotA = k (=0)   <!-- await 之前先讀值 -->
    A->>MQ: 推入續行(用 snapshotA)

    Main->>IIFE: 立即函式執行
    IIFE->>Main: k = 1 並 console.log(1)

    Main->>B: 呼叫 addK('B')
    B->>B: snapshotB = k (=1)   <!-- 這時同步已把 k 改為 1 -->
    B->>MQ: 推入續行(用 snapshotB)

    Note over Main: 同步程式碼結束 → 進入 Microtasks

    MQ->>A: 續行：k = snapshotA + 1 = 0 + 1 = 1
    A->>Main: console.log("Await A: 1")

    MQ->>B: 續行：k = snapshotB + 1 = 1 + 1 = 2
    B->>Main: console.log("Await B: 2")

    MQ->>A: 執行 A.then → console.log("A:", k=2)
    MQ->>B: 執行 B.then → console.log("B:", k=2)

    Note over Main: 清空 microtasks 後，輪到 macrotask

    TQ->>Main: 執行 setTimeout：k = 2 + 1 = 3
    Main->>Main: console.log(3)
```