---
title: "Day12 - JS 的原型鍊又在說什麼故事？"
sidebar_label: "Day12 - JS 的原型鍊又在說什麼故事？"
unlisted: true
...
為何說原型鍊是 JS 接近底層的概念？  
我們看著下方的 code 來思索一個問題：為什麼 `nameList` 這個 array 可以用 `.length` 來取得內容長度？
...
啊，因為 `nameList` 就是一個 array 呀，然後 `.length` 就是 array 的一個操縱方法呀~。  
...
為何說原型鍊跟 OOP 概念很像？  
簡單來說其實 JS 底層有一個屬於每個物件的原型 (prototype)，這個原型就像是 OOP 的 class。  
...
所以為何 `nameList` 可以使用 `.length`？追本溯源是因為它的媽媽 `Array.prototype` 提供了這個屬性給它。
...
舉個例來說，把這段 code 拿去跑會得到 `undefined`：
...
這是因為 `user` 這個物件，從它開始往上找原型鍊，路過了它的媽媽、阿嬤都沒找到 `length` 這個屬性，那自然而然用不了。  
熟悉 TS 開發的，如果寫過聯合型別，如以下範例：

```ts
const score: string | number = 100

score.length
```
會看到 TS 說 `Property 'length' does not exist on type 'number'.`，這就是原型鏈的體現。  
因為 `number` 那條原型鍊上沒有 `length` 這個屬性，所以 TS 會報錯。  
