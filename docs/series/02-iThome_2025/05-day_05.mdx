---
title: Day05 - 所以 arrow function 是指被箭射中的函式嗎？
sidebar_label: "Day05 - 所以 arrow function 是指被箭射中的函式嗎？"
unlisted: true
tags: [JavaScript, arrow function, function, this, hoisting]
keywords: [JavaScript, arrow function, function, this, hoisting]
---

函式 (function)，其他語言比較喜歡稱為 method，都可以，但我個人還是講 method 比較多 (至少這樣腦袋比較不會因為換語言打結 www)。  
總而言之，這篇文章我會以 method 來稱呼他 (字比較少)。  
...
:::warning
我猜一定有人反骨去試用 `var` 寫函式表達式，然後來說：「瞧，也是 ReferenceError 啊！」
```javascript
sayHi() // ReferenceError: sayHi is not defined
...
啊，well，那我會建議往下看 terminal 噴的第二個錯誤。  
用 `const` 宣告的情況下，錯誤會是：
```
...
畢竟提升式會直接噴 error 的，而 `this` 的指向性問題用錯就是隱藏式 bug。
...
最單純的情況下，`function` 宣告的函式，誰呼叫它，`this` 就指向誰家物件。
