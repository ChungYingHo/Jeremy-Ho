...
sidebar_label: "Day08 - 面試很愛考的閉包 (Closure) 到底是怎麼一回事？"
...
幾年前碰 Dart 的時候我再自己的筆記裡是這樣紀錄 closure：
...
但 `outer` 的作用域預備銷毀，理應 `outerVar` 也會被銷毀，但因為 `inner` 這個 closure 的存在，`outerVar` 被帶到了closure `inner` 裡面，其生命週期被延長。  
