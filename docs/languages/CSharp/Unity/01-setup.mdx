...
推上 github 的步驟遵守[這篇文章](https://jeremyho.tw/docs/notes/Env/git)即可，重點是 `.gitignore` 的設定，Unity 初始專案其實很大包，裡面有很多東西是不想推上去的，所以在 `git init` 之前需要先建立 `.gitignore` 檔案。  
For unity 的 `.gitignore` 檔案設定直接去官方的 [Unity gitignore](https://github.com/github/gitignore/blob/main/Unity.gitignore) copy 即可。