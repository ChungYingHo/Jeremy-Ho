...
### 繼承 (Inheritance)
既然叫做繼承，那就一定有個親緣關係存在。  
OOP 的世界裡，其實不需要每個 class 都要開發者詳細定義內部的屬性與方法，只要是有相似性的 class，就可以把它們相似的部分抽出來定義在一個**父類別 (Parent Class)** 裡，然後其他**子類別 (Child Class)** 就可以透過**繼承**這個父類別的屬性與方法，來避免重複定義相同的屬性與方法。
