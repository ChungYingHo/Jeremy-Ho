...
1. **不必要的重新渲染**：使用 `useState` 會導致每次更新 interval ID 時都觸發重新渲染，會造成效能浪費。
2. **狀態快照問題**：如果點擊 start 後很迅速地點即 stop 又點擊 start，有機會抓錯 interval ID，這是因為每一次 Render，`useState` 都會產生一次快照，而 `useRef` 則不會。但當然，這個例子中幾乎是遇不到拉。
:::
